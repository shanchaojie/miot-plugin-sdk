...
export default class WasherReport extends React.Component {
  ...
  componentDidMount () {
    ...
    MHPluginSDK.getDevicePropertyFromSrvCache(["power", "start", "status"], (result) => {
      console.log("直接获取==== ", result);
      this.setState({
        power: result['power'] == undefined ? 0 : parseInt(result['power']),
        start: result['start'] == undefined ? 0 : parseInt(result['start']),
        status: result['status'] == undefined ? 0 : parseInt(result['status']),
      });

    });

    //监听属性
    //,"event.forget","event.status_change","event.warn","event.wash_result"
    MHPluginSDK.registerDeviceStatusProps(["prop.power", "prop.start", "prop.status"]);
    this._deviceStatusChangeListener = DeviceEventEmitter.addListener(
      MHPluginSDK.deviceStatusUpdatedEventName,
      (notifition) => {
        MHPluginSDK.getDevicePropertyFromMemCache(["prop.child", "prop.heat", "prop.mode", "prop.model", "prop.ordermin", "prop.power", "prop.start", "prop.status", "prop.usetime", "prop.wash_count"], (result) => {
          // console.log("result==== ",result);
          this.setState({
            power: result['prop.power'] == undefined ? 0 : parseInt(result['prop.power']),
            start: result['prop.start'] == undefined ? 0 : parseInt(result['prop.start']),
            status: result['prop.status'] == undefined ? 0 : parseInt(result['prop.status']),
          })
